...
%  COPYRIGHT NOTICE
%  © 2025 Fajie Wang, Xiaowen Shi. All Rights Reserved.
%   
...
%  [3] Li X, Wang F, Wang R, Zhao S, Yang D. Fundamental solution neural networks for solving inverse Cauchy problems for the Laplace and biharmonic equations.
%      Comput. Math. Appl., 2026, 201: 1-17. https://doi.org/10.1016/j.camwa.2025.09.032
...
[bp, inp, N_flux, S] = Geometry_Data(nb);  
%------------------------------
...
u_exact = @(x,y) 0.25*(x.^2+y.^2).*exp(x).*(sin(y)+cos(y)) + 6.*x.^2; %analytical solution
du_dx = @(x,y) 0.25*exp(x).*(sin(y)+cos(y)).*(x.^2+y.^2+2*x)+ 12*x;
du_dy = @(x,y) 0.25*exp(x).*(2*y.*(sin(y)+cos(y)) + (x.^2+y.^2).* (cos(y)-sin(y)) );
du_dn_exact = @(x,y,n1,n2) n1.*du_dx(x,y) + n2.*du_dy(x,y);

f_exact = @(x,y) 1.6.*exp(x).*(sin(y)+cos(y)) + 12;
%-------------------Boundary conditions-----------------------
[BC] = Boundary_Conditions(bp,N_flux,aerf);
% -----------------------------------------------------
dlBC = dlarray(BC,'CB');
dltheta = dlarray(theta(1,:),'CB');
dlbp_x = dlarray(bp(:,1),'CB');
dlbp_y = dlarray(bp(:,2),'CB');
...
objFun = @(parameters) objectiveFunction(parameters,dltheta,dlbp_x ,dlbp_y,dlBC,parameterNames,parameterSizes,dlflux);
...
disp(['Error_global of u on the whole domain: ',num2str(E_globalS)])
...
function [bp,inp,N_flux,S] = Geometry_Data(nb)
...
function [BC,N_flux_x, N_flux_y] = Boundary_Conditions(bp,N_N,aerf)
global u_exact du_dn_exact Accessible_BC
% ---------------------------------------------
Accessible_BC = length(bp); 
% =========================================================================
%                               Boundary conditions
% =========================================================================
BC = zeros(2*Accessible_BC, 1);
% ---------------------------------------------------
BC_Dirichlet = u_exact(bp(:, 1), bp(:, 2)).*(1+(-1+2*rand(size(bp(:, 2))))*aerf);
BC_Neumann = du_dn_exact(bp(:, 1), bp(:, 2), N_N(:, 1), N_N(:, 2)).*(1+(-1+2*rand(size(bp(:, 2))))*aerf);
% % ---------------------------------------------------
...
parameters.("alafa_1") = initializeOnes([1 nb],'double');
parameters.("alafa_2") = initializeOnes([1 nb],'double');
end
...
function [loss,gradientsV] = objectiveFunction(parametersV,dltheta,dlbp_x,dlbp_y,dlBC,parameterNames,parameterSizes,dlflux)
parametersV = dlarray(parametersV);
parameters = parameterVectorToStruct(parametersV,parameterNames,parameterSizes);
% Evaluate model gradients and loss.
accmodelGradients = dlaccelerate(@modelGradients);
[gradients,loss] = dlfeval(accmodelGradients,parameters,dltheta,dlbp_x,dlbp_y,dlBC,dlflux);
...
function [gradients,loss] = modelGradients(parameters,dltheta,dlbp_x,dlbp_y,dlBC,dlflux)
...
s_x = r.*cos(dltheta);
s_y = r.*sin(dltheta);
r = sqrt((dlbp_x - s_x).^2+(dlbp_y - s_y).^2); 
% =========================================================================
%           Calculate lossU. Enforce Dirichlet boundary conditions
% =========================================================================
G1 = -1./2./pi.*log(r);
G1x = -1./(2.*pi.*r.^(2)).*(dlbp_x - s_x);
G1y = -1./(2.*pi.*r.^(2)).*(dlbp_y - s_y); 

G2 = -1./2./pi.*r.^2.*log(r);
G2x = -1./2./pi.*(2.*log(r)+1).*(dlbp_x - s_x);
G2y = -1./2./pi.*(2.*log(r)+1).*(dlbp_y - s_y);
...
dlflux_x = dlflux(:, 1); 
 dlflux_y = dlflux(:, 2); 
 G1N = G1x.*dlflux_x + G1y.*dlflux_y;
 G2N = G2x.*dlflux_x + G2y.*dlflux_y;
 N_2 = sum(G2N.*alfa_2,2);
 N_1 = sum(G1N.*alfa_1,2);
U_Neumann = N_1+N_2;
...
s_y = r.*sin(theta);
s = [s_x' s_y'];
r = sqrt((S(:,1) - s_x).^2+(S(:,2) - s_y).^2); %得到距离r
G1 = -1./2./pi.*log(r);%得到u*
G2 = -1./2./pi.*r.^2.*log(r);
alfa_1 = parameters.alafa_1;
alfa_2 = parameters.alafa_2;
...
U_all = U_1+U_2;
E_S = u_exact(S(:,1),S(:,2));
E_globalS = (sum(abs(U_all - E_S).^2)./(sum(E_S.^2))).^0.5;
end
