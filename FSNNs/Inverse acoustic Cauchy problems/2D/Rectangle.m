...
ti=linspace(x_scale(1)+dx/2,x_scale(2)-dx/2,nbx);
tii=linspace(x_scale(2)-dx/2,x_scale(1)+dx/2,nbx);
tj=linspace(y_scale(1)+dy/2,y_scale(2)-dy/2,nby);
tjj=linspace(y_scale(2)-dy/2,y_scale(1)+dy/2,nby);

%边界点坐标
A(:,1:nb)=[x_scale(1).*ones(1,nby) ti x_scale(2).*ones(1,nby) tii;tjj y_scale(1).*ones(1,nbx) tj y_scale(2).*ones(1,nbx)];
pn1(1:nb)=[-ones(1,nby) zeros(1,nbx) ones(1,nby) zeros(1,nbx)];
pn2(1:nb)=[zeros(1,nby) -ones(1,nbx) zeros(1,nby) ones(1,nbx)];
pn1=pn1'; pn2=pn2';
...
xp(1:ni)=B(1,1:ni);   %内部点横坐标
yp(1:ni)=B(2,1:ni);   %内部点纵坐标

xp(ni+1:N)=A(1,1:nb);  %边界点横坐标
yp(ni+1:N)=A(2,1:nb);  %边界点纵坐标
% 
% if irand==1
% 
